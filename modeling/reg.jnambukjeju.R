...
#jnam1
#################
exp1<-read.csv("C:/Users/localadmin/Desktop/data/modeling data/exp/jnam1_x.csv")
res1<-read.csv("C:/Users/localadmin/Desktop/data/modeling data/res/jnam1_y.csv")
...
range(exp1$rain_lasting) #rain lasting hour가 0이다.
...
lasso.jnam1<-glmnet(x, y, alpha=1, family='binomial', lambda=cv.lasso$lambda.min,standardize=TRUE, type.measure = 'deviance') #alpha=1 means lasso fit
...
prob<-predict(lasso.jnam1, newx=x.test, type='response')
...
alasso.jnam1<-glmnet(x, y, alpha=1, family='binomial', penalty.factor= 1/abs(best_ridge_coef), lambda=cv.alasso$lambda.min,standardize=TRUE, type.measure = 'deviance')
prob.alasso<-predict(alasso.jnam1, newx=x.test, type='response')
...
#jnam2
############
exp2<-read.csv("C:/Users/localadmin/Desktop/data/modeling data/exp/jnam2_x.csv")
res2<-read.csv("C:/Users/localadmin/Desktop/data/modeling data/res/jnam2_y.csv")
...
range(exp2$rain_lasting) #전남2지역도 rain lasting hour가 0이다.
...
plot(cv.lasso)
#fit the model on the training data
lasso.jnam2<-glmnet(x, y, alpha=1, family='binomial', lambda=cv.lasso$lambda.min,standardize=TRUE, type.measure = 'deviance') #alpha=1 means lasso fit
#display regression coefficients
coef(lasso.jnam2)
x.test<-model.matrix(infected~acid+co_2+ch_4+n_20+utray+dust+avg_temp+low_temp+high_temp+max_rain_hour+day_rain+avg_wind+avg_hum+min_hum+tot_hour_sun+avg_bottom_temp, test.data)[,-1]
prob<-predict(lasso.jnam2, newx=x.test, type='response')
predicted<-ifelse(prob>0.1, 1, 0)
#model accuracy
obs<-test.data$infected
mean(predicted==obs)
...
best_ridge_coef <- as.numeric(coef(cv.ridge, s=cv.ridge$lambda.min))[-1]
cv.alasso<-cv.glmnet(x, y, alpha=1, family='binomial', penalty.factor = 1 / abs(best_ridge_coef),standardize=TRUE, type.measure = 'deviance')
alasso.jnam2<-glmnet(x, y, alpha=1, family='binomial', penalty.factor= 1/abs(best_ridge_coef), lambda=cv.alasso$lambda.min,standardize=TRUE, type.measure = 'deviance')
prob.alasso<-predict(alasso.jnam2, newx=x.test, type='response')
predicted.alasso<-ifelse(prob.alasso>0.1, 1, 0)
#model accuracy
obs<-test.data$infected
mean(predicted.alasso==obs)
...
#jbuk1
############
exp3<-read.csv("C:/Users/localadmin/Desktop/data/modeling data/exp/jbuk_x.csv")
res3<-read.csv("C:/Users/localadmin/Desktop/data/modeling data/res/jbuk_y.csv")
...
plot(cv.lasso)
#fit the model on the training data
lasso.jbuk1<-glmnet(x, y, alpha=1, family='binomial', lambda=cv.lasso$lambda.min,standardize=TRUE, type.measure = 'deviance') #alpha=1 means lasso fit
#display regression coefficients
coef(lasso.jbuk1)
...
prob<-predict(lasso.jbuk1, newx=x.test, type='response')
predicted<-ifelse(prob>0.1, 1, 0)
#model accuracy
obs<-test.data$infected
mean(predicted==obs)
...
best_ridge_coef <- as.numeric(coef(cv.ridge, s=cv.ridge$lambda.min))[-1]
cv.alasso<-cv.glmnet(x, y, alpha=1, family='binomial', penalty.factor = 1 / abs(best_ridge_coef),standardize=TRUE, type.measure = 'deviance')
alasso.jbuk1<-glmnet(x, y, alpha=1, family='binomial', penalty.factor= 1/abs(best_ridge_coef), lambda=cv.alasso$lambda.min,standardize=TRUE, type.measure = 'deviance')
prob.alasso<-predict(alasso.jbuk1, newx=x.test, type='response')
predicted.alasso<-ifelse(prob.alasso>0.1, 1, 0)
obs<-test.data$infected
mean(predicted.alasso==obs)
confusionMatrix(as.factor(predicted.alasso), as.factor(obs))
...
#jeju
############
exp4<-read.csv("C:/Users/localadmin/Desktop/data/modeling data/exp/jeju_x.csv")
res4<-read.csv("C:/Users/localadmin/Desktop/data/modeling data/res/jeju_y.csv")
...
plot(cv.lasso)
#fit the model on the training data
lasso.jeju<-glmnet(x, y, alpha=1, family='binomial', lambda=cv.lasso$lambda.min,standardize=TRUE, type.measure = 'deviance') #alpha=1 means lasso fit
#display regression coefficients
coef(lasso.jeju)
x.test<-model.matrix(infected~acid+co_2+ch_4+n_20+utray+dust+avg_temp+low_temp+high_temp+rain_lasting+max_rain_hour+day_rain+avg_wind+avg_hum+min_hum+tot_hour_sun+avg_bottom_temp, test.data)[,-1]
prob<-predict(lasso.jeju, newx=x.test, type='response')
predicted<-ifelse(prob>0.1, 1, 0)
#model accuracy
obs<-test.data$infected
mean(predicted==obs)
...
best_ridge_coef <- as.numeric(coef(cv.ridge, s=cv.ridge$lambda.min))[-1]
cv.alasso<-cv.glmnet(x, y, alpha=1, family='binomial', penalty.factor = 1 / abs(best_ridge_coef),standardize=TRUE, type.measure = 'deviance')
alasso.jeju<-glmnet(x, y, alpha=1, family='binomial', penalty.factor= 1/abs(best_ridge_coef), lambda=cv.alasso$lambda.min,standardize=TRUE, type.measure = 'deviance')
prob.alasso<-predict(alasso.jeju, newx=x.test, type='response')
predicted.alasso<-ifelse(prob.alasso>0.1, 1, 0)
#model accuracy
obs<-test.data$infected
mean(predicted.alasso==obs)
